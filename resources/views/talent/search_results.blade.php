      ...
      var all_levels = [];
           ...
           if($(window).scrollTop() + $(window).height() == $(document).height()) {         
            ...
            .done(function( data ) {
                ...
                for (var i = results.length - 1; i >= 0; i--) {
                    ...
                    $('#results_for_search').append("<div class='col-md-3'><div class='card-s'><img src='"+results[i].profile_img+"' alt='Avatar' class='sch_img' onerror='imgError(this);'><div class='inner_card_body'><h4><b>"+results[i].name+"</b></h4><p><a><img src='{{URL::asset('img/add_user_icon.png')}}' class='left_add_user'></a><a href=\"/open_conversation?to="+results[i]._id+"\"><img src='{{URL::asset('img/comment.svg')}}' class='rigth_add_user'></a></p></div></div></div>");
                }
            });
            ...
            .done(function( data ) {
                ...
                for (var i = results.length - 1; i >= 0; i--) {
                    //console.log("inside loop: "+i+"  "+results[i].name);
                    $('#results_for_search').append("<div class='col-md-3'><div class='card-s'><img src='"+results[i].profile_img+"' alt='Avatar' class='sch_img'><div class='inner_card_body'><h4><b>"+results[i].name+"</b></h4><p><a><img src='{{URL::asset('img/add_user_icon.png')}}' class='left_add_user'></a><a href=\"/open_conversation?to="+results[i]._id+"\"><img src='{{URL::asset('img/comment.svg')}}' class='rigth_add_user'></a></p></div></div></div>");
                }
            });
            ...
            $.get( "searcht_results", { terms: s, pass_values: loaded_values, qtype:'job_search' } )
            .done(function( data ) {
                var tmp = JSON.parse(data);
                var results = tmp.results;
                for (var i = results.length - 1; i >= 0; i--) {
                    //console.log("inside loop: "+i+"  "+results[i].name);
                    $('#results_for_search').append("<div class='col-md-3'><div class='card-s'><img src='"+results[i].profile_img+"' alt='Avatar' class='sch_img'><div class='inner_card_body'><h4><b>"+results[i].name+"</b></h4><p>"+results[i].title+"</p></div></div></div>");
                }
            });
        ...
        $( document ).ready(function() {
          $('#tbusq').text('Talent');
            ...
            $.get( gvalues.API_URL+"getCategories", {} )
                .done(function( data ) {
                    all_categories = data;
                });

            $.get( gvalues.API_URL+"getLevels", {} )
                .done(function( data ) {
                    all_levels = data;
                });
        });
        ...
        $('#stl').click(function(){
            ...
            $.get( "searcht_results", { terms: s, pass_values: null, qtype:'talent_search' } )
            .done(function( data ) {
                var tmp = JSON.parse(data);
                var results = tmp.results;
        ...
        $('#srec').click(function(){
            ...
            // hjqr
            $.get( "searcht_results", { terms: s, pass_values: null, qtype:'recruiter_search' } )
            .done(function( data ) {
                var tmp = JSON.parse(data);
                var results = tmp.results;
                $('#contenido').append("<div id='results_for_search' class='row'>");
        ...
        $('#sjbs').click(function(){
            ...
            $.get( "searcht_results", { terms: s, pass_values: null, qtype:'job_search' } )
            .done(function( data ) {
                var tmp = JSON.parse(data);
                var results = tmp.results;
                $('#contenido').append("<div id='results_for_search' class='row'>");
                for (var i = results.length - 1; i >= 0; i--) {
                    //console.log("inside loop: "+i+"  "+results[i].name);
                    $('#results_for_search').append("<div class='col-md-3'><div class='card-s'><img src='"+results[i].profile_img+"' alt='Avatar' class='sch_img'><div class='inner_card_body'><h4><b>"+results[i].name+"</b></h4><p><a><img src='{{URL::asset('img/add_user_icon.png')}}' class='left_add_user'></a><a><img src='{{URL::asset('img/comment.svg')}}' class='rigth_add_user'></a></p></div></div></div>");
                }
                $('#contenido').append("</div>");
            });
        });
